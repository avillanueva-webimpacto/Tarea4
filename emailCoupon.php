...
class EmailCoupon extends Module
{
    ...
    public function __construct()
    {
        ...
        $this->displayName = $this->l('Cupón y aviso por email');
        $this->description = $this->l('Tarea número 4-7 de Prestashop para webImpacto');
    ...
    {
        $sqluninstall1=Db::getInstance()->execute('DROP TABLE IF EXISTS '._DB_PREFIX_.'emailcoupon');
    ...
    public function getContent()
    {
        ...
        //$dbcontent=Db::getInstance()->executeS("SELECT `moneydiscount`,`discount`,`codediscount`,`user_id`,`date` from "._DB_PREFIX_."emailcoupon order by `user_id` asc");
        //left outer join query
        $dbcontent=Db::getInstance()->executeS("SELECT ps_emailcoupon.moneydiscount,ps_emailcoupon.discount,ps_emailcoupon.codediscount,ps_emailcoupon.user_id,ps_emailcoupon.date,ps_customer.email from ps_emailcoupon left outer join ps_customer on ps_emailcoupon.user_id=ps_customer.id_customer order by ps_emailcoupon.user_id asc");
    ...
    protected function renderForm()
    {
        ...
        $helper->submit_action = 'guardar'; //submitemailcouponModule
    ...
    protected function postProcess()
    {
        ...
        if (Tools::isSubmit('guardar')) {
            $id_emailcoupon=(int)Tools::getValue('id_emailcoupon');
            ...
            Db::getInstance()->insert('emailcoupon', array(
    ...
    {
        $numrows=Db::getInstance()->getValue(sprintf("SELECT COUNT(*) AS num FROM ps_emailcoupon WHERE user_id= %d"),(int)pSQL($params['customer']->id));
        ...
        if ($numrows == 0) {
            ...
            if ($totalgastado_ > $minimorequerido) {
                ...
                Mail::Send(
                    ...
                    null,
                    true,
                    _PS_ROOT_DIR_.'/modules/emailcoupon/mails',
                    false,
                    null,
                    null,
                    null
            ...
            $moneydiscount=Db::getInstance()->executeS("SELECT `moneydiscount` from `"._DB_PREFIX_."emailcoupon` WHERE `user_id`=".$id_customer."");
            $discount=Db::getInstance()->executeS("SELECT `discount` from `"._DB_PREFIX_."emailcoupon` WHERE `user_id`=".$id_customer."");
            $codediscount=Db::getInstance()->executeS("SELECT `codediscount` from `"._DB_PREFIX_."emailcoupon` WHERE `user_id`=".$id_customer."");
